...
public class MainAppStockPriceProducer {
	...
	public static void main(String[] args) {
		List<String> bovespa = Arrays.asList("PETR4", "ITSA4", "BBAS3", "CIEL3");

		StockPriceProducer stockProducer = StockPriceProducer.getInstance();
		
		for (int i = 0; i < 1000; i++) {
			bovespa.parallelStream()
				.forEach(ticker -> {
					StockPrice stock = new StockPrice(ticker, randomPrice());
